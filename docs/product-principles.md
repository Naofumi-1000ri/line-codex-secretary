...
連絡先メールアドレスなど、登録先には必要でもAIとの会話へ残す必要がない個人情報は、公式フォームへ本人が直接入力する。同じフォームの `[確認]` が入力確定と規約同意を兼ねる場合は、メール入力と `[確認]` を別々の案内にせず、一つの目的としてまとめる。次の確認画面にあるアカウント作成の最終確定は、別の本人判断として案内する。

必須欄が多い画面では、利用者へ空欄を丸投げしない。既存の正規登録情報を同じサービス内の新規リソースへ複写できる場合は、取得元、登録先、複写項目をまとめて示し、「この内容でいいですか？」と一度確認する。了承後はAIがまとめて入力し、不足・変更項目だけを利用者へ依頼する。既存情報の複写と既存リソースへの接続を混同しない。

複写元がない場合も処理を未定義にしない。地域、組織名、用途名、公開プロジェクトURLなど事実性を損なわず決められる項目には提案値を入れる。氏名、住所、電話、メール、法的区分など本人しか確定できない事実は捏造せず、一つの公式フォーム上でまとめて入力してもらう。個別項目ごとに会話を往復せず、送信前に提案値を含む全体を一度確認する。
